...
) 
{
    ...
    *out_master_dev_handle = new_dev_handle;
    ...
    // Validate provided driver task handle
    if (task_func == NULL) {
        STA_RET_PUSH_LOG(STA_C(ERR_ESP_ERR_INVALID_ARG, OWN_m_i2c_add_driver, 0), "Driver task handle is NULL");
        return STA_C(ERR_ESP_ERR_INVALID_ARG, OWN_m_i2c_add_driver, 0);
    }
...
esp_err_t m_i2c_device_present(bool bus, uint8_t device_address) {
    return  i2c_master_probe(m_i2c_get_bus_handle(bus ? 1 : 0), device_address, 100);
}