...
void tca_wrapper_init(void* dev) {
    ...
    tca_preset_cfg(dev, inputs_mask | outputs_mask, inputs_mask, true);
}
...
esp_err_t io_sys_drv_callback_set(bool drv_num, void (*drv_callback)(void* arg), void* arg){
    ...
    tca_register_pin_callback(tca_dev_handle, __builtin_ctz(pin_mask), drv_callback, , arg);
    return ESP_OK;
}

esp_err_t io_sys_usb_callback_set(void (*usb_callback)(void* arg), void* arg){
    tca_register_pin_callback(tca_dev_handle, __builtin_ctz(IO_TCA_PD_INT), usb_callback, TCA_ON_FALLING_EDGE, arg);
    return ESP_OK;
}

esp_err_t io_sys_ina3221_callback_set(void (*ina_callback_c)(void* arg), void* arg_c, void (*ina_callback_w)(void* arg), void* arg_w){
    tca_register_pin_callback(tca_dev_handle, __builtin_ctz(IO_TCA_INA3221_WARN), ina_callback_w, TCA_ON_FALLING_EDGE, arg_w);
    tca_register_pin_callback(tca_dev_handle, __builtin_ctz(IO_TCA_INA3221_CRIT), ina_callback_c, TCA_ON_FALLING_EDGE, arg_c);
    return ESP_OK;
}
