...
emu_result_t block_counter(block_handle_t block) {
    ...
    // CTU (Count Up)
    bool cu_active = block_check_in_true(block, IN_0_CTU);
    if (cu_active) {
        ...
        data->prev_ctu = false;  // Reset edge detector when input goes false
    }

    // CTD (Count Down)
    bool cd_active = block_check_in_true(block, IN_1_CTD);
    if (cd_active) {
        if (data->cfg == CFG_ON_RISING) {
            if (!data->prev_ctd) {
                data->current_val -= data->step;
        ...
        data->prev_ctd = false;  // Reset edge detector when input goes false
    }
