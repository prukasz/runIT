...
class mem_instance_t:
    ...
    def __init__(self, ctx: int, mem_type: mem_types_t, alias: str, data: Any, dims: Optional[List[int]], can_clear: int, is_standalone: int, is_mutable: int, my_block: Optional[Block] = None):
        self.alias = alias
...
class mem_context_t:
    ...
    def add(self, type: mem_types_t, alias: str, data: Any = None, dims: Optional[List[int]] = None, can_clear: int = 1, is_standalone: int = 0, is_mutable: int = 1, my_block=None) -> mem_instance_t:
        """
...
class Mem:
    ...
    def add_instance(self, ctx: int, type: mem_types_t, alias: str, data: Any = None, dims: Optional[List[int]] = None, can_clear: int = 1, is_standalone: int = 0, is_mutable: int = 1, my_block=None):
        """
        ...
        return self.contexts[ctx].add(type=type, alias=alias, data=data, dims=dims, can_clear=can_clear, is_standalone=is_standalone, is_mutable=is_mutable, my_block=my_block)
...
# ---------------------------------------------------------------------------
# Module-level global Mem instance.
# Call set_global_mem(mem) once at startup so that Ref can resolve aliases
# without carrying a mem reference through every constructor.
# ---------------------------------------------------------------------------
_global_mem: Optional['Mem'] = None

def set_global_mem(mem: 'Mem') -> None:
    """Set the module-level Mem instance used by Ref for alias resolution."""
    global _global_mem
    _global_mem = mem

def get_global_mem() -> 'Mem':
    """Return the module-level Mem instance, raising if not yet set."""
    if _global_mem is None:
        raise RuntimeError("Global Mem not set. Call set_global_mem(mem) first.")
    return _global_mem
...
class Ref:
    ...
    def __init__(self, alias: str):
        ...
        self.instance: mem_instance_t = get_global_mem().get_instance(alias)  # Resolve at creation via global Mem
...
def ref_from_str(string: str, mem: Mem) -> Ref:
    ...
    indices, integer literals, and quoted strings inside brackets.
    Supports: table["inner[0]"] where quoted content is parsed recursively.
    """
    s = string.strip()
    pos = 0
    ...
    def parse_ref() -> Ref:
        ...
        while pos < len(s) and s[pos] == '[':
            ...
            # Check for quoted string (e.g., "f[1]")
            if s[pos] == '"':
                pos += 1  # skip opening "
                quote_start = pos
                ...
                while pos < len(s) and s[pos] != '"':
                    pos += 1
                if pos >= len(s):
                    raise ValueError("Unterminated quoted string in index")
                quoted_content = s[quote_start:pos]
                pos += 1  # skip closing "
                # Recursively parse the quoted content
                nested = ref_from_str(quoted_content, mem)
                r.indices.append((False, nested))
            # integer literal (optional leading '-')
            elif s[pos] == '-' or s[pos].isdigit():
                start_idx = pos
                if s[pos] == '-':
                    pos += 1
