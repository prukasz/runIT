...
class BlockFor(Block):
    ...
    Inputs:
        - in_0 (EN): Enable block input (as bool)
        - in_1 (START): (any) Initial iterator value (optional, overrides config)
        - in_2 (LIMIT): (any) Loop end limit (optional, overrides config)
        - in_3 (STEP): (any) Iterator step value (optional, overrides config)
    INFO: Before use Ref converted to float
        
    Outputs:
        - q_0 (ENO): Enable output (Bool) for block chaining 
        - q_1 (ITER): Current iterator value (Float)
    
    Config:
        - chain_len: Number of blocks in the loop body - filled automatically 
