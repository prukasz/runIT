...
class Block:
    ...
    def __init__(self, idx: int, alias: str, block_type: block_types_t, ctx_id, mem: Mem):
        ...
        self.my_dep: List['Block'] = []  # blocks this block depends on
        self.have_me: List['Block'] = []  # blocks that depend on this block

    @property
    def idx(self) -> int:
        """Shortcut for ``cfg.block_idx`` (stays in sync after reindex)."""
        return self.cfg.block_idx

    @property
    def block_type(self) -> int:
        """Shortcut for ``cfg.block_type``."""
        return self.cfg.block_type
    ...
    def add_inputs(self, inputs: List[Optional[Ref]]):
        ...
        for inp in inputs:
            if inp is not None:
                ...
                # If this Ref points to a block output, record the dependency
                producer = inp.instance.my_block
                if producer is not None and producer not in self.my_dep:
                    self.my_dep.append(producer)
                    producer.have_me.append(self)
            else:
    ...
    def add_outputs(self, outputs: List[tuple]):
        ...
        for i, output in enumerate(outputs):
            ...
            new_inst = self.mem.add_instance(self.ctx_id, m_type, f"{self.alias}_q_{i}", dims=dims, can_clear=1, is_standalone=0, is_mutable=0, my_block=self)
    ...
    def pack_connections(self) -> List[bytes]:
        ...
        # inputs: kind_flag=0, outputs: kind_flag=1  (single packet type)
        _append_conn(self.in_conn, 0x00, packet_header_t.PACKET_H_BLOCK_INPUTS)
        _append_conn(self.q_conn, 0x01, packet_header_t.PACKET_H_BLOCK_INPUTS)
