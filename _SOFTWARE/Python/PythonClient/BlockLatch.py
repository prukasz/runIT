"""
BlockLatch - a latch block with configurable behavior (SR or RS)
- Inputs: [EN, SET (as bool), RESET (as bool)]
- Output: Q (bool)
- Configuration: Latch type (SR or RS) : (BloickLatchCfg enum)
"""
...
class BlockLatchCfg(IntEnum):
    ...
    LATCH_SR = 0  # S=1,R=0 -> Q=1; S=0,R=1 -> Q=0; S=R=0 -> hold; S=R=1 -> Q=1 (set dominant)
    LATCH_RS = 1  # R=1,S=0 -> Q=0; R=0,S=1 -> Q=1; R=S=0 -> hold; R=S=1 -> Q=0 (reset dominant)
...
class BlockLatch(Block):
    ...
    def pack_data(self) -> list[bytes]:
        ...
        # Config packet (we send latch type only)
        ...
        # offset appliet to match C struct layout, where latch_type is at offset 1, 0 is used as prev state
