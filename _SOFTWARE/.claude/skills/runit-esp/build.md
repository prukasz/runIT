...
| [pcb_tests.py](scripts/pcb_tests.py) | PCB stage 1 on COM4: status LEDs (TCA 22 / 23 blink), TCA freeze → sync, TCA inputs, ADS7128 voltages (ch7 ≈ 3.18 V), ADS7128 window alerts → event → action for both window modes, PCA9685 PWM / level, AP33772S VBUS / current / PD list | 2026-09-24 PCB: pass |
