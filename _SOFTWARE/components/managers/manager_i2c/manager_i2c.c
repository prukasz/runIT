...
{
    if (task_func == NULL) {return STA_C(ESP_ERR_INVALID_ARG, OWNER_I2C_ADD_DRIVER, 0);}
    ...
    for (int i = 0; i < M_I2C_MAX_DEVICES; i++) {
        if (driver_registry[i].i2_address == i2c_addr && driver_registry[i].bus == bus) {
            ...
            return STA_C(ERR_I2C_DEV_ADDR_CONFLICT, OWNER_I2C_ADD_DRIVER, dev_config.device_address);
        }
    }
    ...
    if (slot == -1) {
        ...
        return STA_C(ERR_I2C_DEV_REG_FULL, OWNER_I2C_ADD_DRIVER, M_I2C_MAX_DEVICES);
    }

    i2c_master_bus_handle_t target_bus_handle = m_i2c_get_bus_handle(bus ? 1 : 0);
    i2c_master_dev_handle_t new_dev_handle;
    
    STA_RET_ON_ESP_ERR(i2c_master_bus_add_device(target_bus_handle, &dev_config, &new_dev_handle), OWNER_I2C_ADD_DRIVER, dev_config.device_address);
    STA_RET_ON_ESP_ERR(i2c_master_bus_add_device(target_bus_handle, &dev_config, &new_dev_handle), OWNER_I2C_ADD_DRIVER, dev_config.device_address);
    ...
    *out_master_dev_handle = new_dev_handle;
    ...
    ESP_LOGI(TAG, "m_i2c_add_driver: registered driver id=%d task_handle=%p dev_handle=%p", driver_registry[slot].id, (void*)driver_registry[slot].task_handle, driver_registry[slot].dev_handle);
    if (is_periodic) {
...
status_rep_t m_i2c_enqueue_aperiodic_job(uint8_t id) {
    ...
    return STA_C(ERR_I2C_DEV_NOT_FOUND, OWNER_I2C_ADD_DRIVER, id);
}
