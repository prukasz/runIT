...
status_rep_t manager_io_register_new_port(io_port_dispatch_t *port_dispatch, uint8_t* out_port_id){
    if (next_free_port >= MAX_IO_PORTS) {
        return STA_W(ERR_MANAGER_IO_NO_FREE_PORT, OWNER_MANAGER_IO, 0);
    }
...
status_rep_t sys_gpio_set_mode(uint8_t port_id, uint64_t pin_mask, uint32_t mode){
    if (port_id >= MAX_IO_PORTS) return STA_W(ERR_MANAGER_IO_INVALID_PORT, OWNER_MANAGER_IO, 0);
    if (port_registry[port_id].mode_func == NULL) return STA_W(ERR_MANAGER_IO_FUNC_NULL, OWNER_MANAGER_IO, 0);
    if (global_io_is_protected && (pin_mask & port_registry[port_id].protected_pins)) return STA_W(ERR_MANAGER_IO_PIN_PROTECTED, OWNER_MANAGER_IO, 0);
...
status_rep_t sys_gpio_set_level(uint8_t port_id, uint64_t pin_mask, bool level){
    if (port_id >= MAX_IO_PORTS) return STA_W(ERR_MANAGER_IO_INVALID_PORT, OWNER_MANAGER_IO, 0);
    if (port_registry[port_id].set_func == NULL) return STA_W(ERR_MANAGER_IO_FUNC_NULL, OWNER_MANAGER_IO, 0);
    if (global_io_is_protected && (pin_mask & port_registry[port_id].protected_pins)) return STA_W(ERR_MANAGER_IO_PIN_PROTECTED, OWNER_MANAGER_IO, 0);
...
status_rep_t sys_gpio_read_level(uint8_t port_id, uint64_t pin_mask, bool* level){
    if (port_id >= MAX_IO_PORTS) return STA_W(ERR_MANAGER_IO_INVALID_PORT, OWNER_MANAGER_IO, 0);
    if (port_registry[port_id].read_func == NULL) return STA_W(ERR_MANAGER_IO_FUNC_NULL, OWNER_MANAGER_IO, 0);
    return port_registry[port_id].read_func(pin_mask, level);
}

status_rep_t sys_gpio_toggle(uint8_t port_id, uint64_t pin_mask){
    if (port_id >= MAX_IO_PORTS) return STA_W(ERR_MANAGER_IO_INVALID_PORT, OWNER_MANAGER_IO, 0);
    if (port_registry[port_id].toggle_func == NULL) return STA_W(ERR_MANAGER_IO_FUNC_NULL, OWNER_MANAGER_IO, 0);
    if (global_io_is_protected && (pin_mask & port_registry[port_id].protected_pins)) return STA_W(ERR_MANAGER_IO_PIN_PROTECTED, OWNER_MANAGER_IO, 0);
...
status_rep_t sys_gpio_register_callback(uint8_t port_id, uint64_t pin_mask, uint32_t mode, void (*callback)(void* arg), void* arg){
    if (port_id >= MAX_IO_PORTS) return STA_W(ERR_MANAGER_IO_INVALID_PORT, OWNER_MANAGER_IO, 0);
    if (port_registry[port_id].callback_add_func == NULL) return STA_W(ERR_MANAGER_IO_FUNC_NULL, OWNER_MANAGER_IO, 0);
    if (global_io_is_protected && (pin_mask & port_registry[port_id].protected_pins)) return STA_W(ERR_MANAGER_IO_PIN_PROTECTED, OWNER_MANAGER_IO, 0);
...
status_rep_t sys_io_set_pwm_duty(uint8_t port_id, uint64_t pin_mask, uint32_t duty_cycle){
    if (port_id >= MAX_IO_PORTS) return STA_W(ERR_MANAGER_IO_INVALID_PORT, OWNER_MANAGER_IO, 0);
    if (port_registry[port_id].pwm_set_duty_func == NULL) return STA_W(ERR_MANAGER_IO_FUNC_NULL, OWNER_MANAGER_IO, 0);
    if (global_io_is_protected && (pin_mask & port_registry[port_id].protected_pins)) return STA_W(ERR_MANAGER_IO_PIN_PROTECTED, OWNER_MANAGER_IO, 0);
...
status_rep_t sys_io_set_pwm_freq(uint8_t port_id, uint64_t pin_mask, uint32_t freq_hz){
    if (port_id >= MAX_IO_PORTS) return STA_W(ERR_MANAGER_IO_INVALID_PORT, OWNER_MANAGER_IO, 0);
    if (port_registry[port_id].pwm_set_freq_func == NULL) return STA_W(ERR_MANAGER_IO_FUNC_NULL, OWNER_MANAGER_IO, 0);
    if (global_io_is_protected && (pin_mask & port_registry[port_id].protected_pins)) return STA_W(ERR_MANAGER_IO_PIN_PROTECTED, OWNER_MANAGER_IO, 0);
...
status_rep_t sys_io_adc_read(uint8_t port_id, uint64_t pin_mask, uint32_t* out_mv, uint8_t max_results_num){
    if (port_id >= MAX_IO_PORTS) return STA_W(ERR_MANAGER_IO_INVALID_PORT, OWNER_MANAGER_IO, 0);
    if (port_registry[port_id].adc_read_func == NULL) return STA_W(ERR_MANAGER_IO_FUNC_NULL, OWNER_MANAGER_IO, 0);
...
status_rep_t sys_io_adc_register_callback(uint8_t port_id, uint64_t pin_mask, void* adc_int_config){
    if (port_id >= MAX_IO_PORTS) return STA_W(ERR_MANAGER_IO_INVALID_PORT, OWNER_MANAGER_IO, 0);
    if (port_registry[port_id].adc_callback_add_func == NULL) return STA_W(ERR_MANAGER_IO_FUNC_NULL, OWNER_MANAGER_IO, 0);
    if (global_io_is_protected && (pin_mask & port_registry[port_id].protected_pins)) return STA_W(ERR_MANAGER_IO_PIN_PROTECTED, OWNER_MANAGER_IO, 0);
