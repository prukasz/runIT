 ...
 *
 * Private to sys_errors. It only *fills a buffer*: SE_send_error_raw() passes
 * the packet to the registered sink (SE_register_sink()), which transmits it.
 *
 * ## Wire format
 *
 * The encoder does not include a class byte. The transport behind the sink
 * (runit binds the errors data connector) adds its own framing header;
 * receivers consume that byte before decoding this format.
 *
 * @code
 *   +--------- packet header (6 B) ---------+
 ...
 * | `tag` | `err_tag_e` value: stable ID, high byte = owner domain (0x00 = base tags). |
 * | `owner` | `sys_owner_e` value. |
 * | `payload` | The node's payload struct copied verbatim. |
 *
 * Nodes are emitted **outermost first** (the `SE_REPORT` frame), each
 ...
 *   SE_release(sys_data_connector_send(SYS_DATA_CONNECTOR_ERRORS, pkt, pkt_len));
 * }
 * @endcode
 */
static inline SE_MUST_USE err_h enc_sys_errors_encode_chain(err_h chain, uint8_t* out_buf, size_t out_max, size_t* out_len) {
  SE_CHECK_NOT_NULL(chain);
  SE_CHECK_NOT_NULL(out_buf);
  SE_CHECK_NOT_NULL(out_len);
  if (out_max < ENC_SYS_ERRORS_MIN_BUF) {
    SE_FAIL(ERR_INTERFACE_ENC_BUF_TOO_SMALL, .got = (uint32_t)out_max, .need = ENC_SYS_ERRORS_MIN_BUF);
  }
  ...
  if (node_count == 0) {
    SE_FAIL(ERR_INTERFACE_ENC_BUF_TOO_SMALL, .got = (uint32_t)out_max,
