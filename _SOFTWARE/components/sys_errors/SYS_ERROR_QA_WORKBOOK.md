...
- [x ] **A:** 100% direct and synchronous on the calling thread. Delete `sys_error_handler_task` and `s_err_queue` completely.
...
- [x ] **A:** Serial prints inline synchronously; BLE packet is handed to the existing non-blocking TX ring buffer (drops if full, zero blocking).
...
- [ x ] **A:** `SE_ORIGIN_CALL(call)` is the top-level boundary. Add `SE_REPORT_DEV_ERR(dev_id, tag, ...)` for explicit reporting.
...
- [ x ] **A:** Latch the fault status in VM (`vm_exec_fault_latch`) and trigger a task notification to wake the main task to execute the full freeze.
...
- [ x ] **A:** A static boolean `s_in_policy` drops any nested policy calls. Any error tagged `ERR_DEV_FAULT_RESPONSE_FAILED` is logged to serial and never re-evaluated.
...
- [ x ] **A:** Four roles: `SYSTEM_CRITICAL` (power/safety), `IMPORTANT` (actuators/motors), `USER` (UI/sensors), `TEST` (fixtures/mocks).
...
- [ x ] **A:** Dynamic instance config: Specified per-instance during install via its config struct (`cfg->role`). (e.g. an expander can be CRITICAL on mainboard, but USER on an expansion header).
...
- [ x ] **A:** Yes, allow dynamic role modification via a contract packet.
...
- [ x ] **A:** Tier 1: Device adapter's `classify_error` hook runs first. If NULL or returning `UNSPECIFIED`, fall back to Tier 2 (Central Role + Tag Matrix).
...
- [ x ] **A:** Remove both booleans completely. Replace with `role`, `health`, and optional `action_overrides[3]` (0 = use role default action).
...
- [ x ] **A:** Yes, any communication error immediately trips the assigned role severity.
...
- [ x ] **B:** Inside each individual driver/adapter (driver retries $N$ times before returning an error).
- **Your Answer / Notes:**
...
- **Your Answer / Notes:**
  extend device state then  with health
...
- [ x] **A:** Self-healing: automatically clear health back to `OK` on the first successful transaction.
...
- [ x ] **B:** (1) Freeze all devices immediately &rarr; (2) Stop VM &rarr; (3) Run emergency action &rarr; (4) Latch fault.
...
- [ x ] **A:** Localized isolation: call `sys_device_freeze(dev_id)` for that device only. The rest of the board and VM keep running.
...
- [ x ] **A:** Block execution fails cleanly with an error code; user VM script error-handling handles it.
...
- [ x ] **A:** Log to serial and latch `ERR_DEV_FAULT_RESPONSE_FAILED`; board remains in safe state.
...
- [ x] **A:** Escalates to `CRITICAL`: triggers full safe state (stops VM, freezes all devices, latches fault).
...
- **Your Answer / Notes:**
  per block fault policy - default as Warning
...
- [ x ] **A:** `CRITICAL`: system safe state.
...
- **Your Answer / Notes:**
  let invke custom not yet implemented power handler
...
- [ x ] **B:** Immediately trigger `CRITICAL` safe state without recovery.
- **Your Answer / Notes:**
...
- **Your Answer / Notes:**
  two conditions: set remotely flag that going offline - all is okay - flag not turned on - critical stop (aka connection not lost at purpose)
...
- [ x ] **A:** `CRITICAL`: immediate safe state.
...
- [ x ] **A:** `uint8_t device_id; uint8_t role; uint8_t action_overrides[3];`
...
- **Your Answer / Notes:**
  error chain is exactly digital twin of erorr chian
...
- **Your Answer / Notes:**
- only one action binded per severity - packet decides scope + id x3
...
Additional concerns: error chains double escalations: we have device call inside block - now device fails - and instantly gets handled - now what blocks does - reports chain with itself on top so error chain can be fully linked towards block? or we then doesnt handle erorrs below? only stores that block failed due to system call fail. or we dont report error in system and rely on error reported via block call - then process whole chain? how then classify error severity ?
in case of critical system failure  - how do we sync - make aware vm of it. I want to have implemented error callback to vm - so errors can be handled as objects inside vm code. how do we make this so we can chose what errors are getting pushed as callback to VM?
is this feasable to make basic error handling per device - specific configurable via defice directly: read error - warning - write error - critical, power error - critical - invalid parameter  - info, no memeory - warnbing and so on
