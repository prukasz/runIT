 ...
 */
static __inline status_rep_t rik_tx_send(const void* data, size_t len, uint32_t interface, bool return_on_full){
    status_rep_t rep = STA_OK;
    if (!data || !len || !rik_buff_tx){ return STA_C(ESP_ERR_INVALID_ARG, OWNER_RIK_TX_SEND, 0);}
    if (interface == 0){
        if(_rik_ble_active){ 
            rep = STA_FROM_ESP(m_ble_tx_enqueue(rik_buff_tx, (const uint8_t*)data, len, 0), OWNER_RIK_TX_SEND, return_on_full);
 ...
 * @return status_rep_t.e_code 0 on success, ERR_INTERFACE_UNAVAILABLE, ERR_TX_BUFFER_FULL, ESP_ERR_INVALID_ARG
 */
#define RIK_TX_NO_WAIT(data_ptr, len) rik_tx_send((const void*)(data_ptr), (len), 0, 0)
/**
 ...
 * @return status_rep_t.e_code 0 on success, ERR_INTERFACE_UNAVAILABLE, ERR_TX_BUFFER_FULL, ESP_ERR_INVALID_ARG
 */
#define RIK_TX_WAIT(data_ptr, len) rik_tx_send((const void*)(data_ptr), (len), 0, 1)
 ...
 * @return status_rep_t.e_code 0 on success, ERR_INTERFACE_UNAVAILABLE, ERR_TX_BUFFER_FULL, ESP_ERR_INVALID_ARG
 */
#define RIK_TX_BLE_NO_WAIT(data_ptr, len) rik_tx_send((const void*)(data_ptr), (len), 1, 0)
 ...
 * @return status_rep_t.e_code 0 on success, ERR_INTERFACE_UNAVAILABLE, ERR_TX_BUFFER_FULL, ESP_ERR_INVALID_ARG
 */
#define RIK_TX_BLE_WAIT(data_ptr, len) rik_tx_send((const void*)(data_ptr), (len), 1, 1)
