...
{
    tps55289_handle_t handle = malloc(sizeof(_tps55289_data_t));
    if (!handle)
    {
    ...
    memset(handle, 0, sizeof(_tps55289_data_t));
...
esp_err_t tps55289_sync_all_registers(tps55289_handle_t handle)
{
    ...
    for (uint8_t reg = 0; reg < TPS55289_REG_MAX; reg++) {
        esp_err_t err = _tps55289_read(handle, reg, &temp);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Failed to sync register 0x%02X", reg);
            return err;
        }
    }
    ESP_LOGI(TAG, "TPS55289 (0x%02X) register cache synced.", handle->i2c_device_config.device_address);
    return ESP_OK;
}

// --- NOWE FUNKCJE DO KONTROLI ZASILACZA ---
...
esp_err_t tps55289_set_current_limit(tps55289_handle_t handle, bool enable, uint16_t limit_ma)
{
    ...
    // Napięcie na rezystorze przy podanym prądzie
    // V = I * R
