...
static esp_err_t _tca_update_port(tca6424a_handle_t handle, uint8_t port){
    return (esp_err_t)i2c_master_transmit(handle->i2c_dev_handle, (uint8_t[]){TCA6424A_REG_OUTPUT_PORT0 + port, handle->output[port]}, 2, 10);
}
...
static esp_err_t _tca_update_config(tca6424a_handle_t handle){
    ...
    // return tca_transmit(handle->i2c_dev_handle, (uint8_t[]){TCA6424A_REG_CONFIG_PORT0 | TCA6424A_AUTO_INCREMENT, handle->config[0], handle->config[1], handle->config[2]}, 4, 10);
    // #else
    return i2c_master_transmit(handle->i2c_dev_handle, (uint8_t[]){TCA6424A_REG_CONFIG_PORT0 | TCA6424A_AUTO_INCREMENT, handle->config[0], handle->config[1], handle->config[2]}, 4, 100);
    //#endif
}
...
static esp_err_t _tca_update_polarity(tca6424a_handle_t handle){
    ...
    // #else
    return i2c_master_transmit(handle->i2c_dev_handle, (uint8_t[]){TCA6424A_REG_POLARITY_PORT0 | TCA6424A_AUTO_INCREMENT, handle->polarity_cfg[0], handle->polarity_cfg[1], handle->polarity_cfg[2]}, 4, 100);
    // #endif
}
