  ...
  while (1) {
    if (R_QUEUE_RECEIVE(s_callback_queue, &event, WAIT_FOREVER)) {
      ...
      if (event.head.callback_type == CALLBACK_IO) {
        ESP_LOGI(TAG, "Received IO Event: Device ID %u, Pin %u, Event %u, Val %ld",
                 event.event.io.device_id, event.event.io.pin_id,
                 event.event.io.trigger_event, (long)event.event.io.trigger_value);
      } else if (event.head.callback_type == CALLBACK_PWR) {
        ESP_LOGI(TAG, "Received PWR Event: Device ID %u, Channel %u, Event %u, Val %ld",
                 event.event.pwr.device_id, event.event.pwr.channel_id,
                 event.event.pwr.trigger_event, (long)event.event.pwr.trigger_value);
      }
    }
  }
}
