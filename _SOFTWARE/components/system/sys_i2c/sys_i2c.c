...
i2c_master_bus_handle_t sys_i2c_get_bus_handle(bool bus_num) { return (bus_num == 0) ? bus_handle_0 : bus_handle_1; }
...
esp_err_t sys_i2c_master_transmit(void *device_handle, const uint8_t *write_buffer, size_t write_size) {
  ...
  if (err != ESP_OK) {
    ESP_LOGE("i2c_transmit", "Transaction failed %s", esp_err_to_name(err));
...
esp_err_t sys_i2c_master_transmit_receive(void *device_handle, const uint8_t *write_buffer, size_t write_size, uint8_t *read_buffer, size_t read_size) {
  ...
  if (err != ESP_OK) {
    ESP_LOGE("i2c_trans_recv", "Transaction failed %s", esp_err_to_name(err));
