...
status_rep_t cfg_pwr_process_packet(const uint8_t* packet_data, uint16_t packet_len){
    switch(packet_data[0]){
        ...
        case CFG_PWR_TYPE_REG_SETTINGS:{
            ...
            #if CONFIG_CONNECT_TPS55289_0
                ESP_LOGI(TAG, "configuring VREG0, voltage %d mV, current %d mA", settings.voltage_reg_0_mV, settings.current_limit_reg_0_mA);
            ...
            #endif
            #if CONFIG_CONNECT_TPS55289_1
            ESP_LOGI(TAG, "configuring VREG1, voltage %d mV, current %d mA", settings.voltage_reg_1_mV, settings.current_limit_reg_1_mA);
        ...
        case CFG_PWR_TYPE_SUPPLY:{
            ...
            CHECK_AND_RETURN(sys_pwr_set_bus_current_warning(RIK_CHANNEL_TOTAL, settings.input_current_warning_mA));
            CHECK_AND_RETURN(sys_pwr_set_bus_current_critical(RIK_CHANNEL_TOTAL, settings.input_current_critical_mA));
        ...
        case CFG_PWR_TYPE_TEST_SET_PD:{
            ...
            CHECK_AND_RETURN(sys_pwr_set_pd_voltage_current(settings.pd_voltage_mv, settings.pd_current_ma));
