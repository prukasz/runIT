...
/**
 * @brief Parse and assign block connection (input or output)
 * @param data packet buff (skip header)
 * @param packet_length packet buff len (-header len)
 * @param emu_code_handle code handle where block connection will be assigned
 * @note Packet [uint16_t block_idx] + [uint8_t kind (0=in, 1=out)] + [uint8_t conn_idx] + mem_access_t parse packet 
 */
emu_result_t emu_block_parse_connection(const uint8_t *data, const uint16_t packet_length, void * emu_code_handle);
