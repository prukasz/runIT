...
emu_result_t emu_block_parse_connection(const uint8_t *data, const uint16_t el_cnt, void * emu_code_handle){
    emu_code_handle_t code = (emu_code_handle_t)emu_code_handle;
    uint16_t block_idx = parse_get_u16(data, 0);
    uint8_t kind     = data[2];   // 0x00 = input, 0x01 = output
    uint8_t conn_idx = data[3];
    mem_access_t *access = NULL;
    uint16_t parse_idx = 4;
    uint16_t index = 0;
    emu_err_t res_code = emu_mem_parse_access(data + parse_idx, el_cnt - parse_idx, &index, &access);
    if(res_code != EMU_OK){RET_ED(res_code, block_idx, 0, "Failed to parse %s access for block %d conn %d, error %s", kind ? "output" : "input", block_idx, conn_idx, EMU_ERR_TO_STR(res_code));}
    if(block_idx >= code->total_blocks){RET_ED(EMU_ERR_BLOCK_INVALID_PARAM, block_idx, 0, "Invalid block idx %d for connection parse", block_idx);}
    block_handle_t block = code->blocks_list[block_idx];
    if(kind == 0x00){
        if(conn_idx >= block->cfg.in_cnt){RET_ED(EMU_ERR_BLOCK_INVALID_PARAM, block_idx, 0, "Invalid input idx %d for block %d", conn_idx, block_idx);}
        block->inputs[conn_idx] = access;
    } else {
        if(conn_idx >= block->cfg.q_cnt){RET_ED(EMU_ERR_BLOCK_INVALID_PARAM, block_idx, 0, "Invalid output idx %d for block %d", conn_idx, block_idx);}
        block->outputs[conn_idx] = access;
    }
    return EMU_RESULT_OK();
}
