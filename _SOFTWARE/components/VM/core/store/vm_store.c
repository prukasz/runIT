...
err_h vm_store_open(uint32_t total_size, const uint16_t counts[VM_REG_CNT]) {
  ...
  for (int r = 0; r < VM_REG_CNT; r++) {
    ...
    if (!items) {
      SE_FAIL(ERR_VM_ALLOC_EXHAUSTED, .requested = reg_size, .remaining = g_vm_store.arena.capacity - g_vm_store.arena.offset);
    }
  ...
  *out = NULL;
  ...
  if (id != VM_ID_NONE) {
    if ((unsigned)r >= VM_REG_CNT) {
      SE_FAIL(ERR_VM_REG_OOB, .kind = (uint8_t)r, .id = id, .count = 0);
    }
    g = &g_vm_store.reg[r];
    if (id >= g->count || g->items == NULL) {
      SE_FAIL(ERR_VM_REG_OOB, .kind = (uint8_t)r, .id = id, .count = g->count);
    }
    if (g->items[id] != NULL) {  // each id is written exactly once per load
      SE_FAIL(ERR_VM_REG_DUP, .kind = (uint8_t)r, .id = id);
    }
  }

  void* p = arena_carve(&g_vm_store.arena, size);
  if (!p) {
    SE_FAIL(ERR_VM_ALLOC_EXHAUSTED, .requested = size, .remaining = g_vm_store.arena.capacity - g_vm_store.arena.offset);
  }
  memset(p, 0, size);
