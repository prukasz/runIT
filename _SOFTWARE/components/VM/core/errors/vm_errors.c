...
__attribute__((noinline)) err_h vm_err_depth(uint16_t id) {
  SE_FAIL_OWNED(OWNER_VM_ACCESSOR, ERR_VM_ACCESSOR_DEPTH_EXCEEDED, .id = id);
}
...
// built by hand, not SE_FAIL: the payload carries a copied string, and
...
__attribute__((noinline)) err_h vm_block_err_pin_missing(uint16_t block_idx, uint8_t pin_id, bool is_out) {
  SE_FAIL_OWNED(OWNER_VM_BLOCK, ERR_VM_BLOCK_PIN_MISSING, .block_idx = block_idx, .pin_id = pin_id, .is_out = is_out ? 1 : 0);
}

__attribute__((noinline)) err_h vm_block_err_pin_unlinked(uint16_t block_idx, uint8_t pin_id, bool is_out) {
  SE_FAIL_OWNED(OWNER_VM_BLOCK, ERR_VM_BLOCK_PIN_UNLINKED, .block_idx = block_idx, .pin_id = pin_id, .is_out = is_out ? 1 : 0);
}
