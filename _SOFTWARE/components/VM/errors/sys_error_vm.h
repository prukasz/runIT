...
#define LOG_BODY_ERR_VM_EVENT_OVERFLOW(p, out, out_size)                                                     \
  snprintf((out), (out_size), "vm event buffer full at %u per cycle: %u dropped, latest from event domain %u", \
...
#define SE_CHECK_NOT_NULL_OWNED(owner, ptr)              \
  do {                                                   \
    if ((ptr) == NULL) {                                 \
      SE_FAIL_OWNED((owner), ERR_NULL_PTR, 0);        \
    }                                                    \
  } while (0)
...
#define SE_CHECK_IF_ALLOCATED_OWNED(owner, ptr)          \
  do {                                                   \
    if ((ptr) == NULL) {                                 \
      SE_FAIL_OWNED((owner), ERR_BASE_NO_MEM, 0);     \
    }                                                    \
  } while (0)
