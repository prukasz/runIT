...
#define FAULT_ACTION_NONE 12
#define FAULT_ACTION_LOW 13
#define FAULT_ACTION_MEDIUM 14
#define FAULT_ACTION_HIGH 15
#define FAULT_ACTION_CRITICAL 16
...
static err_h fault_action_none(void) {
  s_fault_ctx.actions[SYS_DEV_ERR_NONE]++;
  return NULL;
}
...
void test_device_fault_policy(void) {
  ...
  (void)sys_actions_bind_static(FAULT_ACTION_NONE, fault_action_none);
  (void)sys_actions_bind_static(FAULT_ACTION_LOW, fault_action_low);
  (void)sys_actions_bind_static(FAULT_ACTION_MEDIUM, fault_action_medium);
  (void)sys_actions_bind_static(FAULT_ACTION_HIGH, fault_action_high);
  (void)sys_actions_bind_static(FAULT_ACTION_CRITICAL, fault_action_critical);
  const uint8_t actions[] = {FAULT_ACTION_NONE, FAULT_ACTION_LOW, FAULT_ACTION_MEDIUM,
                             FAULT_ACTION_HIGH, FAULT_ACTION_CRITICAL};
  ck("device error actions configured",
     sys_device_set_error_handling(cfg.device_id, SYS_DEV_IMPORTANCE_CRITICAL, actions) == NULL);
  ...
  ck("low fault invokes only its configured action",
     sys_device_report_error(cfg.device_id, low) == NULL &&
  ...
  ck("medium fault invokes only its configured action",
     sys_device_report_error(cfg.device_id, medium) == NULL &&
  ...
  ck("high fault invokes only its configured action",
     sys_device_report_error(cfg.device_id, high) == NULL &&
  ...
  ck("reconfigure device importance to LOW",
     sys_device_set_error_handling(cfg.device_id, SYS_DEV_IMPORTANCE_LOW, actions) == NULL);
  vm_exec_set_mode(VM_RUN_RUNNING);
  ...
  ck("reconfigure device importance to NONE",
     sys_device_set_error_handling(cfg.device_id, SYS_DEV_IMPORTANCE_NONE, actions) == NULL);
  ...
  ck("critical error on NONE importance device is ignored (no halt, no latch)",
     sys_device_report_error(cfg.device_id, crit_none) == NULL &&
         vm_exec_mode() == VM_RUN_RUNNING && !vm_exec_fault_status().latched);

  /* Re-enable for action failure test */
  (void)sys_device_set_error_handling(cfg.device_id, SYS_DEV_IMPORTANCE_HIGH, actions);
  (void)sys_actions_bind_static(FAULT_ACTION_LOW, fault_action_fail);
  ...
  (void)sys_actions_bind_static(FAULT_ACTION_NONE, NULL);
  (void)sys_actions_bind_static(FAULT_ACTION_LOW, NULL);
  (void)sys_actions_bind_static(FAULT_ACTION_MEDIUM, NULL);
  (void)sys_actions_bind_static(FAULT_ACTION_HIGH, NULL);
