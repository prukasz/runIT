...
void test_header_helpers(void) {
  ...
  for (unsigned i = 0; i < sizeof(types) / sizeof(types[0]); i++) {
    ...
    if (!selftest_ok(vm_obj_create(&o, VM_ID_NONE, &h3, NULL)) || !o) {
      counts_ok = false;
      continue;
    }
  ...
  bool          elem_ok = selftest_ok(vm_obj_create(&eo, VM_ID_NONE, &h32, NULL)) && eo;
  if (elem_ok) {
...
void test_conversion(void) {
  ...
  uint8_t in_u8 = 200;
  float   out_f = 0;
  ck("U8 200 -> float 200", selftest_ok(VM_OBJ_SET_SCALAR(in_u8, &a_u8)) && selftest_ok(VM_OBJ_SCALAR_GET(out_f, &a_u8)) && out_f == 200.0f);

  int32_t in_i = -5;
  out_f        = 0;
  ck("I32 -5 -> float -5", selftest_ok(VM_OBJ_SET_SCALAR(in_i, &a_i32)) && selftest_ok(VM_OBJ_SCALAR_GET(out_f, &a_i32)) && out_f == -5.0f);
  ...
  float   in_f  = 179.6f;
  int32_t out_i = 0;
  ck("float 179.6 -> int 180 (rounds, not truncates)", selftest_ok(VM_OBJ_SET_SCALAR(in_f, &a_f)) && selftest_ok(VM_OBJ_SCALAR_GET(out_i, &a_f)) && out_i == 180);

  in_f  = -3.7f;
  out_i = 0;
  ck("float -3.7 -> int -4", selftest_ok(VM_OBJ_SET_SCALAR(in_f, &a_f)) && selftest_ok(VM_OBJ_SCALAR_GET(out_i, &a_f)) && out_i == -4);

  /* Narrowing saturates to the destination range on reads and writes. */
  in_f           = 300.0f;
  uint8_t out_u8 = 0;
  ck("float 300 -> uint8 saturates to 255", selftest_ok(VM_OBJ_SET_SCALAR(in_f, &a_f)) && selftest_ok(VM_OBJ_SCALAR_GET(out_u8, &a_f)) && out_u8 == 255);
  ...
  bool     in_b  = true;
  uint32_t out_b = 0;
  ck("bool true -> U8 object reads 1", selftest_ok(VM_OBJ_SET_SCALAR(in_b, &a_u8)) && selftest_ok(VM_OBJ_SCALAR_GET(out_b, &a_u8)) && out_b == 1);

  char     in_c  = 'A';
  uint32_t out_c = 0;
  ck("char 'A' -> U8 object reads 65", selftest_ok(VM_OBJ_SET_SCALAR(in_c, &a_u8)) && selftest_ok(VM_OBJ_SCALAR_GET(out_c, &a_u8)) && out_c == 65);

  uint8_t in_u8b  = 0xC3;
  float   out_u8f = 0;
  ck("U8 0xC3 -> float 195", selftest_ok(VM_OBJ_SET_SCALAR(in_u8b, &a_u8)) && selftest_ok(VM_OBJ_SCALAR_GET(out_u8f, &a_u8)) && out_u8f == 195.0f);

  // Storing wider than the object holds saturates on the way in.
  uint32_t in_u32 = 0x12345678;
  uint8_t  narrow = 0;
  ck("U32 stored into U8 object saturates", selftest_ok(VM_OBJ_SET_SCALAR(in_u32, &a_u8)) && selftest_ok(VM_OBJ_SCALAR_GET(narrow, &a_u8)) && narrow == UINT8_MAX);

  narrow = 73;
  ck("NULL scalar accessor preserves output", !selftest_ok(VM_OBJ_SCALAR_GET(narrow, NULL)) && narrow == 73);
  ck("NULL scalar setter fails", !selftest_ok(VM_OBJ_SET_SCALAR((uint8_t)1, NULL)));
  vm_accessor_t missing = {.id = UINT16_MAX};
  ck("missing scalar object preserves output", !selftest_ok(VM_OBJ_SCALAR_GET(narrow, &missing)) && narrow == 73);
  vm_obj_payload_t failed_payload = {.ptr = &narrow, .count = 1, .type = VM_OBJ_U8};
  ck("NULL payload accessor clears descriptor", !selftest_ok(vm_obj_get_payload(&failed_payload, NULL)) && !failed_payload.ptr && !failed_payload.count);
  ck("NULL payload output fails", !selftest_ok(vm_obj_get_payload(NULL, &a_u8)));
  ...
  in_u32 = 7;
  SE_release(VM_OBJ_SET_SCALAR(in_u32, &a_u32));
  vm_obj_payload_t praw = {.ptr = NULL, .count = 0, .type = VM_OBJ_STR, ._pad = 0};
  uint32_t     vraw = 999;
  ck("payload_read of NULL ptr reports error and preserves output", !selftest_ok(VM_PAYLOAD_GET_VAL(vraw, praw)) && vraw == 999);
  ...
  ck("malformed cached read returns error", !selftest_ok(VM_OBJ_SCALAR_GET(vraw, &malformed_cache)) && vraw == 999);
  ck("malformed cached write returns error", !selftest_ok(VM_OBJ_SET_SCALAR(in_u32, &malformed_cache)));
  ref_index = (vm_index_t){.kind = 255, .value = 0};
  ck("unknown index kind cannot silently address element zero", !selftest_ok(VM_OBJ_SCALAR_GET(vraw, &indirect)) && vraw == 999);
  ...
  indirect.id            = 3;
  ck("unterminated missing name safely returns an error", name_parent && !selftest_ok(VM_OBJ_SCALAR_GET(vraw, &indirect)) && vraw == 999);
  ref_index.name = NULL;
  ck("NULL accessor name safely returns an error", !selftest_ok(VM_OBJ_SCALAR_GET(vraw, &indirect)));
  ck("quiet clear rejects NULL", !selftest_ok(vm_obj_clear_quiet(NULL)));
  ck("quiet clear rejects pointer payload", !selftest_ok(vm_obj_clear_quiet(name_parent)));
}
...
void test_resolution(void) {
  ...
  *(float*)leaf->payload  = 1.25f;
  ck("link child into parent slot 1", selftest_ok(vm_obj_link_direct(parent, 1, leaf)));

  // literal index
  static const vm_index_t    lit2[] = {{.kind = VM_IDX_LITERAL, .value = 2}};
  static const vm_accessor_t a_lit  = {.id = 0, .count = 1, .indices = lit2};
  uint32_t                   got    = 0;
  ck("literal index reads arr[2] == 30", selftest_ok(VM_OBJ_SCALAR_GET(got, &a_lit)) && got == 30);
  ...
  got                                = 0;
  ck("by-ref index reads arr[sel=2] == 30", selftest_ok(VM_OBJ_SCALAR_GET(got, &a_dyn)) && got == 30);
  *(uint8_t*)sel->payload = 0;
  got                     = 0;
  ck("by-ref follows sel=0 -> 10 (live, not cached)", selftest_ok(VM_OBJ_SCALAR_GET(got, &a_dyn)) && got == 10);
  ...
  float                      f       = 0;
  ck("name index finds child at slot 1", selftest_ok(VM_OBJ_SCALAR_GET(f, &a_named)) && f == 1.25f);

  // failure modes
  static const vm_index_t    oob[] = {{.kind = VM_IDX_LITERAL, .value = 9}};
  static const vm_accessor_t a_oob = {.id = 0, .count = 1, .indices = oob};
  ck("index past end -> OOB", !selftest_ok(VM_OBJ_SCALAR_GET(got, &a_oob)));

  static const vm_accessor_t a_badid = {.id = 999, .count = 0, .indices = NULL};
  ck("unknown root id -> UNKNOWN_ID", !selftest_ok(VM_OBJ_SCALAR_GET(got, &a_badid)));
  ...
  ck("descend through unlinked slot -> NULL_OBJ", !selftest_ok(VM_OBJ_SCALAR_GET(f, &a_null)));
  ...
  ck("chain through non-PTR -> TYPE_MISMATCH", !selftest_ok(VM_OBJ_SCALAR_GET(got, &a_mismatch)));
  ...
  ck("name index on scalar array -> TYPE_MISMATCH", !selftest_ok(VM_OBJ_SCALAR_GET(got, &a_name_scalar)));
  ...
  *(float*)ro->payload               = 9.5f;
  float rov                          = 0.0f;
  ck("read from non-mutable object succeeds", selftest_ok(VM_OBJ_SCALAR_GET(rov, &a_ro)) && rov == 9.5f);
  float wv = 1.0f;
  ck("write to non-mutable object -> NOT_MUTABLE", !selftest_ok(VM_OBJ_SET_SCALAR(wv, &a_ro)));
  ...
  bool                       got_payload = selftest_ok(vm_obj_get_payload(&arrp, &w_arr));
  ck("get_payload on whole array gives count 4", got_payload && arrp.count == 4 && arrp.type == VM_OBJ_U32);
  uint32_t sum = 0;
  for (uint16_t i = 0; i < arrp.count; i++) {
    uint32_t el = 0;
    ck("array element read succeeds", selftest_ok(VM_PAYLOAD_GET_VAL(el, vm_payload_get_at(arrp, i))));
    sum += el;
  }
  ...
  bool           built     = selftest_ok(vm_accessor_create(&chain[11], s_acc_id++, 1, 0));
  for (int i = 10; i >= 0 && built; i--) {
    built = selftest_ok(vm_accessor_create(&chain[i], s_acc_id++, 0, 1)) && selftest_ok(vm_accessor_set_ref(chain[i], 0, chain[i + 1]));
  }
  ck("built a 12-deep by-ref chain", built);
  if (built) {
    got = 0;
    ck("by-ref chain past MAX_DEPTH -> DEPTH_EXCEEDED", !selftest_ok(VM_OBJ_SCALAR_GET(got, chain[0])));
  }
}
...
void test_nested(void) {
  ...
  *(float*)leaf_y->payload = 2.5f;
  ...
  bool linked = selftest_ok(vm_obj_link_direct(root, 0, br_a)) && selftest_ok(vm_obj_link_direct(root, 1, br_b)) && selftest_ok(vm_obj_link_direct(br_a, 0, leaf_x)) && selftest_ok(vm_obj_link_direct(br_a, 1, leaf_y)) && selftest_ok(vm_obj_link_direct(br_b, 0, leaf_z)) && selftest_ok(vm_obj_link_direct(grid, 0, row0)) &&
                selftest_ok(vm_obj_link_direct(grid, 1, row1)) && selftest_ok(vm_obj_link_direct(grid, 2, row2));
  ...
  uint32_t                   u        = 0;
  ck("root[0][0][1] == 200 (3-level literal chain)", selftest_ok(VM_OBJ_SCALAR_GET(u, &a_deep)) && u == 200);
  ...
  float                      f         = 0;
  ck("root[0][\"yval\"][0] == 2.5 (mixed literal/name)", selftest_ok(VM_OBJ_SCALAR_GET(f, &a_mixed)) && f == 2.5f);
  ...
  u                                    = 0;
  ck("root[1][\"zdata\"][2] == 3 (other branch)", selftest_ok(VM_OBJ_SCALAR_GET(u, &a_bpath)) && u == 3);
  ...
  u                                   = 0;
  ck("grid[1][2] == 12 (jagged 2D)", selftest_ok(VM_OBJ_SCALAR_GET(u, &a_grid)) && u == 12);
  ...
  vm_obj_h                   h       = NULL;
  ck("get_obj(root[0]) -> branch_a", selftest_ok(vm_obj_get_obj(&h, &a_r0)) && h == br_a);
  h = NULL;
  ck("get_obj(root[0][0]) -> leaf_x", selftest_ok(vm_obj_get_obj(&h, &a_r00)) && h == leaf_x);
  ...
  uint32_t sum = 0;
  if (selftest_ok(vm_obj_get_obj(&h, &a_r00)) && h) {
    vm_obj_payload_t p = vm_make_payload(h);
    for (uint16_t i = 0; i < p.count; i++) {
      uint32_t el = 0;
      ck("nested element read succeeds", selftest_ok(VM_PAYLOAD_GET_VAL(el, vm_payload_get_at(p, i))));
      sum += el;
    }
  }
  ...
  leaf_x->head.f.upd = 0;
  uint32_t w         = 999;
  ck("write through root[0][0][1]", selftest_ok(VM_OBJ_SET_SCALAR(w, &a_deep)) && xv[1] == 999);
  ...
  ck("relink leaf_x under branch_b too", selftest_ok(vm_obj_link_direct(br_b, 0, leaf_x)));
  ...
  u                                    = 0;
  ck("root[1][0][1] reaches the same leaf -> 200", selftest_ok(VM_OBJ_SCALAR_GET(u, &a_alias)) && u == 200);
  w = 555;
  u = 0;
  ck("write via one path is visible from the other", selftest_ok(VM_OBJ_SET_SCALAR(w, &a_alias)) && selftest_ok(VM_OBJ_SCALAR_GET(u, &a_deep)) && u == 555);
  ...
  ck("OOB at level 2 -> error", !selftest_ok(VM_OBJ_SCALAR_GET(u, &a_oob2)));
  ...
  ck("chaining past a scalar leaf -> TYPE_MISMATCH", !selftest_ok(VM_OBJ_SCALAR_GET(u, &a_far)));
  ...
  ck("name miss at level 2 -> NAME_NOT_FOUND", !selftest_ok(VM_OBJ_SCALAR_GET(f, &a_badname)));
  ...
  ck("re-point root[0] at branch_b", selftest_ok(vm_obj_link_direct(root, 0, br_b)));
  u = 0;
  ck("root[0][0][1] now reads through branch_b", selftest_ok(VM_OBJ_SCALAR_GET(u, &a_deep)) && u == 555);
  ck("root[0][\"yval\"] no longer resolves after re-link", !selftest_ok(VM_OBJ_SCALAR_GET(f, &a_mixed)));
}
...
void test_mutation(void) {
  ...
  uint32_t v = 77;
  ck("set_scalar_direct writes element 2", selftest_ok(VM_OBJ_SET_SCALAR_AT_IDX(v, a, 2)) && av[2] == 77);
  ck("set_scalar_direct sets upd", a->head.f.upd == 1);
  ck("set_scalar_direct rejects index past end", !selftest_ok(VM_OBJ_SET_SCALAR_AT_IDX(v, a, 9)));
  ck("set_scalar_direct rejects non-mutable", !selftest_ok(VM_OBJ_SET_SCALAR_AT_IDX(v, ro, 0)));
  ...
  bool      copied = selftest_ok(vm_obj_copy_content(&w_a, &w_b));
  ...
  ck("copy_content count mismatch -> COPY_MISMATCH", !selftest_ok(vm_obj_copy_content(&w_a, &w_small)));
  ck("copy_content type mismatch -> COPY_MISMATCH", !selftest_ok(vm_obj_copy_content(&w_a, &w_other)));
  ck("copy_content into non-mutable -> NOT_MUTABLE", !selftest_ok(vm_obj_copy_content(&w_a, &w_ro)));
  ...
  ck("copy_content on a cell copied onto itself is a no-op", selftest_ok(vm_obj_copy_content(&w_cell, &w_cell)));
  ck("copy_content still refuses a pointer against a value", !selftest_ok(vm_obj_copy_content(&w_cell, &w_a)) && !selftest_ok(vm_obj_copy_content(&w_a, &w_cell)));

  // link
  ck("link_direct points cell at leaf", selftest_ok(vm_obj_link_direct(cell, 0, leaf)) && *(vm_obj_h*)cell->payload == leaf);
  ck("link_direct rejects non-PTR target", !selftest_ok(vm_obj_link_direct(a, 0, leaf)));
  ck("link_direct rejects index past end", !selftest_ok(vm_obj_link_direct(cell, 5, leaf)));
  ...
  *(vm_obj_h*)cell->payload              = NULL;
  ck("vm_obj_link via accessors points cell at leaf", selftest_ok(vm_obj_link(&w_leaf, &w_cell_slot)) && *(vm_obj_h*)cell->payload == leaf);
  ck("vm_obj_link rejects a non-PTR owner slot", !selftest_ok(vm_obj_link(&w_leaf, &w_a_whole)));
}
...
void test_block_api(void) {
  ...
  ck("get_in(0) returns the wired accessor", selftest_ok(vm_block_get_in(&got_in, blk, 0)) && got_in == &a_in);
  ck("get_in(1) unwired -> PIN_UNLINKED", !selftest_ok(vm_block_get_in(&got_in, blk, 1)));
  ck("get_in(5) absent -> PIN_MISSING", !selftest_ok(vm_block_get_in(&got_in, blk, 5)));

  vm_obj_h got_out = NULL;
  ck("get_out(0) returns the bound object", selftest_ok(vm_block_get_out(&got_out, blk, 0)) && got_out == out);
  ck("get_out(3) absent -> PIN_MISSING", !selftest_ok(vm_block_get_out(&got_out, blk, 3)));
  ...
  *(float*)child_temp->payload = 21.5f;
  ...
  SE_release(vm_obj_link_direct(ptr_msg, 0, child_temp));
...
static void test_edge_block(void) {
  ...
  ck("edge block rejects undersized custom_len", !selftest_ok(vm_block_create(&bad_b, 98, &(vm_block_cfg_t){.block_idx = 42, .block_type = VM_BLK_EDGE, .custom_len = 10})));
...
static void test_timer_block(void) {
  ...
  ck("timer block rejects undersized custom_len", !selftest_ok(vm_block_create(&bad_tb, 99, &(vm_block_cfg_t){.block_idx = 43, .block_type = VM_BLK_TIMER, .custom_len = 16})));
...
void test_obj_construction(void) {
  ...
  ck("create rejects VM_OBJ_NONE", !selftest_ok(vm_obj_create(&o, VM_ID_NONE, &h, NULL)) && o == NULL);
  h.d.obj_t = 12;  // inside the 4-bit field, outside the width table
  ck("create rejects a type past the width table", !selftest_ok(vm_obj_create(&o, VM_ID_NONE, &h, NULL)));
  ...
  /* A zero payload would allocate a header whose payload pointer aliases the
     ...
     overwrite them. */
  h = hd(VM_OBJ_U32, 0);
  ck("create rejects a zero payload -> OBJ_EMPTY", !selftest_ok(vm_obj_create(&o, VM_ID_NONE, &h, NULL)) && o == NULL);
  ...
  h              = hd(VM_OBJ_U32, 2);
  h.payload_size = 5;
  ck("create rejects a payload that is not a whole number of elements", !selftest_ok(vm_obj_create(&o, VM_ID_NONE, &h, NULL)) && o == NULL);
  ...
  h             = hd(VM_OBJ_PTR, 1);
  h.f.retentive = 1;
  ck("create rejects retentive PTR", !selftest_ok(vm_obj_create(&o, VM_ID_NONE, &h, NULL)));

  h = hd(VM_OBJ_U8, 1);
  // there is no arena parameter to reject any more -- the store owns it
  ck("create rejects a NULL out pointer", !selftest_ok(vm_obj_create(NULL, VM_ID_NONE, &h, NULL)));
  ck("create rejects a NULL head", !selftest_ok(vm_obj_create(&o, VM_ID_NONE, NULL, NULL)));
  ...
  h.f.upd_resetable = 1;
  h.f.retentive     = 1;
  ck("every flag round-trips into the header", selftest_ok(vm_obj_create(&o, VM_ID_NONE, &h, "f")) && o && o->head.f.mutable && o->head.f.upd_resetable && o->head.f.retentive && o->head.f.tagged && o->head.f.upd == 0);
  ...
  ck("create overrides the flags a caller does not own", selftest_ok(vm_obj_create(&forced, VM_ID_NONE, &h, NULL)) && forced && forced->head.f.dynamic == 0 && forced->head.f.upd == 0 && forced->head.f.tagged == 0 && !vm_obj_is_dynamic(forced));

  vm_obj_h plain = NULL;
  h              = hd(VM_OBJ_U8, 1);
  ck("an untagged object clears `tagged` and keeps name_size 0", selftest_ok(vm_obj_create(&plain, VM_ID_NONE, &h, NULL)) && plain && plain->head.f.tagged == 0 && plain->head.d.name_size == 0);

  // the longest tag a 4-bit name_size can describe
  vm_obj_h max  = NULL;
  h             = hd(VM_OBJ_U32, 2);
  h.d.name_size = 15;
  ck("15-char name is accepted", selftest_ok(vm_obj_create(&max, VM_ID_NONE, &h, "abcdefghijklmno")) && max);
  uint8_t     tl  = 0;
  ...
  ck("arena exhaustion reports and leaves the handle NULL", !selftest_ok(vm_obj_create(&big, VM_ID_NONE, &hbig, NULL)) && big == NULL);
  ...
  ck("the largest describable shape still fails on the pool, not the field", !selftest_ok(vm_obj_create(&big, VM_ID_NONE, &hbig, NULL)));
}
...
void test_names_and_accessor_build(void) {
  ...
  *(uint32_t*)c_dup->payload  = 44;
  ...
  bool wired = selftest_ok(vm_obj_link_direct(bag, 1, c_anon)) && selftest_ok(vm_obj_link_direct(bag, 2, c_temp)) && selftest_ok(vm_obj_link_direct(bag, 3, c_long)) && selftest_ok(vm_obj_link_direct(bag, 5, c_dup));
  ...
  uint32_t v = 0;
  ck("exact tag matches past a NULL and an untagged slot", selftest_ok(VM_OBJ_SCALAR_GET(v, &a_exact)) && v == 11);
  v = 0;
  ck("a longer tag is reachable by its full name", selftest_ok(VM_OBJ_SCALAR_GET(v, &a_full)) && v == 22);
  ...
  /* Length is compared as well as bytes. Without that, "tem" would match
     ...
     by-name access. */
  ck("a prefix of a tag does not match", !selftest_ok(VM_OBJ_SCALAR_GET(v, &a_prefix)));
  ck("a tag that is a prefix of the query does not match", !selftest_ok(VM_OBJ_SCALAR_GET(v, &a_longer)));
  ck("an empty query matches nothing", !selftest_ok(VM_OBJ_SCALAR_GET(v, &a_empty)));
  ck("a query longer than any tag can be is refused before the scan", !selftest_ok(VM_OBJ_SCALAR_GET(v, &a_over)));
  ...
  v = 0;
  ck("the lowest matching slot wins when a tag repeats", selftest_ok(VM_OBJ_SCALAR_GET(v, &a_exact)) && v == 11);

  // ---- accessors built through the construction API, not as static structs
  vm_accessor_t* acc  = NULL;
  bool           made = selftest_ok(vm_accessor_create(&acc, s_acc_id++, 0, 2)) && acc && selftest_ok(vm_accessor_set_name(acc, 0, "temperature", 11)) && selftest_ok(vm_accessor_set_literal(acc, 1, 0));
  ck("built an accessor through the construction API", made && acc->count == 2 && acc->indices != NULL);
  v = 0;
  ck("a built accessor resolves like a static one", made && selftest_ok(VM_OBJ_SCALAR_GET(v, acc)) && v == 22);
  ...
  ck("set_literal past the declared index count -> ACC_INDEX_OOB", !selftest_ok(vm_accessor_set_literal(acc, 2, 0)));
  ck("set_ref past the declared index count -> ACC_INDEX_OOB", !selftest_ok(vm_accessor_set_ref(acc, 9, acc)));
  ck("set_ref rejects a NULL target", !selftest_ok(vm_accessor_set_ref(acc, 0, NULL)));
  ck("set_name rejects a 16-char name", !selftest_ok(vm_accessor_set_name(acc, 0, "0123456789abcdef", 16)));
  v = 0;
  ck("the rejected setters left index 0 intact", selftest_ok(VM_OBJ_SCALAR_GET(v, acc)) && v == 22);

  vm_accessor_t* whole = NULL;
  ck("a zero-index accessor allocates no index array", selftest_ok(vm_accessor_create(&whole, s_acc_id++, 1, 0)) && whole && whole->count == 0 && whole->indices == NULL);
  ck("setting an index on a zero-index accessor -> ACC_INDEX_OOB", !selftest_ok(vm_accessor_set_literal(whole, 0, 0)));
  ...
  void* p = NULL;
  ck("alloc rejects an id past the registry", !selftest_ok(vm_store_alloc(&p, VM_REG_OBJ, 32, 8)) && p == NULL);
  ck("alloc rejects a second binding of the same id", !selftest_ok(vm_store_alloc(&p, VM_REG_OBJ, 1, 8)));
  ck("alloc rejects a bound accessor id", !selftest_ok(vm_store_alloc(&p, VM_REG_ACC, 0, 8)));
  ck("VM_ID_NONE allocates without binding", selftest_ok(vm_store_alloc(&p, VM_REG_OBJ, VM_ID_NONE, 8)) && p != NULL);
  ck("a free id binds and is then readable", selftest_ok(vm_store_alloc(&p, VM_REG_OBJ, 20, 8)) && vm_store_get(VM_REG_OBJ, 20) == p);
  ...
  ck("resolution fails closed against a detached store", !selftest_ok(VM_OBJ_SCALAR_GET(v, &a_exact)));
  ck("alloc against a detached store -> REG_OOB", !selftest_ok(vm_store_alloc(&p, VM_REG_OBJ, 0, 8)));
}
...
void test_access_edges(void) {
  ...
  av[2]        = 25;
  av[3]        = 33;
  ck("link arr into box[0]", selftest_ok(vm_obj_link_direct(box, 0, arr)));
  ...
  vm_obj_h h = NULL;
  ck("get_obj on a whole PTR object returns the container", selftest_ok(vm_obj_get_obj(&h, &w_box)) && h == box);
  h = NULL;
  ck("get_obj on a whole scalar object returns it", selftest_ok(vm_obj_get_obj(&h, &w_arr)) && h == arr);

  // a chainless write has no element to land on, so it takes the first
  uint32_t v = 42;
  ck("write via a count-0 accessor lands on element 0", selftest_ok(VM_OBJ_SET_SCALAR(v, &w_arr)) && av[0] == 42);
  ...
  ck("a scalar write into a PTR object is refused", !selftest_ok(VM_OBJ_SET_SCALAR(v, &w_box)));
  ck("the refused write left the link intact", *(vm_obj_h*)box->payload == arr);
  ck("set_scalar_direct into a PTR element is refused", !selftest_ok(VM_OBJ_SET_SCALAR_AT_IDX(v, box, 0)));

  ck("set_scalar_direct rejects a NULL object", !selftest_ok(vm_internal_set_scalar_direct(NULL, 0, (vm_val_t){.u32 = 1}, VM_OBJ_U32)));
  ck("link_direct rejects a NULL child", selftest_ok(vm_obj_link_direct(box, 1, arr)) && !selftest_ok(vm_obj_link_direct(box, 1, NULL)));
  ck("link_direct rejects a non-mutable cell", !selftest_ok(vm_obj_link_direct(ro_box, 0, arr)));
  ...
  *(float*)fsel->payload = 2.7f;
  uint32_t got           = 0;
  ck("a float index rounds to element 3", selftest_ok(VM_OBJ_SCALAR_GET(got, &a_by_f)) && got == 33);
  ...
  *(int32_t*)isel->payload = -1;
  ck("a negative by-ref index is out of range, not wrapped", !selftest_ok(VM_OBJ_SCALAR_GET(got, &a_by_i)));
  ...
  *(int32_t*)isel->payload = 65536;
  ck("index 65536 is out of range, not truncated to 0", !selftest_ok(VM_OBJ_SCALAR_GET(got, &a_by_i)));
  *(int32_t*)isel->payload = 65537;
  ck("index 65537 is out of range, not truncated to 1", !selftest_ok(VM_OBJ_SCALAR_GET(got, &a_by_i)));
  ...
  ck("copy_content copies a single element into a scalar object", selftest_ok(vm_obj_copy_content(&a_arr2, &w_one)) && *(uint32_t*)one->payload == 25);
  ck("copy_content refuses element -> whole array (count mismatch)", !selftest_ok(vm_obj_copy_content(&a_arr2, &w_arr)));
  static const vm_accessor_t a_missing = {.id = 900, .count = 0, .indices = NULL};
  ck("copy_content propagates an unresolvable source", !selftest_ok(vm_obj_copy_content(&a_missing, &w_one)));
  ...
  *(uint32_t*)one->payload = 2;
  ...
  bool           ok8   = idx1 && selftest_ok(vm_accessor_create(&d8[7], s_acc_id++, 2, 0));  // reads `one`
  for (int i = 6; i >= 1 && ok8; i--) {
    ok8 = selftest_ok(vm_accessor_create(&d8[i], s_acc_id++, 7, 1)) && selftest_ok(vm_accessor_set_ref(d8[i], 0, d8[i + 1]));
  }
  ok8 = ok8 && selftest_ok(vm_accessor_create(&d8[0], s_acc_id++, 1, 1)) && selftest_ok(vm_accessor_set_ref(d8[0], 0, d8[1]));
  got = 0;
  ck("a by-ref chain exactly at MAX_DEPTH still resolves", ok8 && selftest_ok(VM_OBJ_SCALAR_GET(got, d8[0])) && got == 15);

  vm_accessor_t* d9[9] = {0};
  bool           ok9   = idx1 && selftest_ok(vm_accessor_create(&d9[8], s_acc_id++, 2, 0));
  for (int i = 7; i >= 1 && ok9; i--) {
    ok9 = selftest_ok(vm_accessor_create(&d9[i], s_acc_id++, 7, 1)) && selftest_ok(vm_accessor_set_ref(d9[i], 0, d9[i + 1]));
  }
  ok9 = ok9 && selftest_ok(vm_accessor_create(&d9[0], s_acc_id++, 1, 1)) && selftest_ok(vm_accessor_set_ref(d9[0], 0, d9[1]));
  ck("one level past MAX_DEPTH is refused", ok9 && !selftest_ok(VM_OBJ_SCALAR_GET(got, d9[0])));
  ...
  bool                       bt     = true;
  uint8_t                    b_out  = 0;
  ck("bool source stores as B and reads back 1", selftest_ok(VM_OBJ_SET_SCALAR(bt, &w_flag)) && selftest_ok(VM_OBJ_SCALAR_GET(b_out, &w_flag)) && b_out == 1);
  ...
  uint32_t z = 7;
  ck("payload_at past the end returns error and preserves output", !selftest_ok(VM_PAYLOAD_GET_VAL(z, vm_payload_get_at(vm_make_payload(arr), 99))) && z == 7);
}
...
void test_strings(void) {
  ...
  uint8_t c = 0;
  ck("indexed read gives the character, not 0", selftest_ok(VM_OBJ_SCALAR_GET(c, &a_s1)) && c == 'e');

  char wc = 'a';
  ck("a plain char writes through as a character", selftest_ok(VM_OBJ_SET_SCALAR(wc, &a_s1)) && s->payload[1] == 'a');
  ck("chars round-trip through the accessor layer", selftest_ok(VM_OBJ_SCALAR_GET(c, &a_s1)) && c == 'a');
  s->payload[1] = 'e';

  // walking the buffer, which is how any string block reads one
  vm_obj_payload_t sp       = {.ptr = NULL, .count = 0, .type = VM_OBJ_NONE, ._pad = 0};
  bool         resolved = selftest_ok(vm_obj_get_payload(&sp, &w_s));
  ck("whole-object accessor yields the buffer", resolved && sp.type == VM_OBJ_STR && sp.count == 8);
  char out[9] = {0};
  for (uint16_t i = 0; resolved && i < sp.count; i++) {
    uint8_t ch = 0;
    ck("string element read succeeds", selftest_ok(VM_PAYLOAD_GET_VAL(ch, vm_payload_get_at(sp, i))));
    out[i] = (char)ch;
  }
  ...
  ck("copy_content copies a whole string", selftest_ok(vm_obj_copy_content(&w_s, &w_s2)) && memcmp(s2->payload, "hello", 5) == 0);
  ck("copy_content refuses a different length", !selftest_ok(vm_obj_copy_content(&w_s, &w_short)));
  ...
  ck("copy_content refuses STR -> U8 despite equal widths", !selftest_ok(vm_obj_copy_content(&w_s, &w_bytes)));

  ck("writing past the end of a string is refused", !selftest_ok(VM_OBJ_SET_SCALAR_AT_IDX(wc, s, 8)));
  ck("writing the last character is accepted", selftest_ok(VM_OBJ_SET_SCALAR_AT_IDX(wc, s, 7)) && s->payload[7] == 'a');
}
...
void test_resolution_cache(void) {
  ...
  ck("link fixtures wired", selftest_ok(vm_obj_link_direct(box, 0, arr)));

  // the two shapes that qualify
  vm_accessor_t* whole = NULL;
  vm_accessor_t* elem  = NULL;
  bool           built = selftest_ok(vm_accessor_create(&whole, s_acc_id++, 1, 0)) && selftest_ok(vm_accessor_create(&elem, s_acc_id++, 1, 1)) && selftest_ok(vm_accessor_set_literal(elem, 0, 2));
  ...
  uint32_t v = 0;
  ck("cached read matches the value", selftest_ok(VM_OBJ_SCALAR_GET(v, elem)) && v == 102);
  ck("cached write lands and sets upd", selftest_ok(VM_OBJ_SET_SCALAR(((uint32_t)777), elem)) && ((uint32_t*)arr->payload)[2] == 777 && arr->head.f.upd);

  vm_obj_payload_t p = {0};
  ck("cached whole-object payload has the full count", selftest_ok(vm_obj_get_payload(&p, whole)) && p.count == 4 && p.ptr == arr->payload);
  ...
  bool           ro_built = selftest_ok(vm_accessor_create(&roacc, s_acc_id++, 3, 1)) && selftest_ok(vm_accessor_set_literal(roacc, 0, 0));
  ck("read-only accessor caches", ro_built && vm_accessor_cache_build(roacc));
  ck("cached write to a non-mutable object is still refused", ro_built && !selftest_ok(VM_OBJ_SET_SCALAR(((uint32_t)5), roacc)));
  ...
  bool           deep_built = selftest_ok(vm_accessor_create(&deep, s_acc_id++, 0, 2)) && selftest_ok(vm_accessor_set_literal(deep, 0, 0)) && selftest_ok(vm_accessor_set_literal(deep, 1, 1));
  ck("two-level chain refuses to cache", deep_built && !vm_accessor_cache_build(deep));

  vm_accessor_t* named       = NULL;
  bool           named_built = selftest_ok(vm_accessor_create(&named, s_acc_id++, 0, 1)) && selftest_ok(vm_accessor_set_name(named, 0, "x", 1));
  ck("by-name index refuses to cache", named_built && !vm_accessor_cache_build(named));

  vm_accessor_t* oob       = NULL;
  bool           oob_built = selftest_ok(vm_accessor_create(&oob, s_acc_id++, 1, 1)) && selftest_ok(vm_accessor_set_literal(oob, 0, 99));
  ck("out-of-range literal refuses to cache", oob_built && !vm_accessor_cache_build(oob));
  ck("an uncached out-of-range accessor still reports OOB", oob_built && !selftest_ok(VM_OBJ_SCALAR_GET(v, oob)));
  ...
  ck("relink to alt", selftest_ok(vm_obj_link_direct(box, 0, alt)));
  uint32_t after = 0;
  ck("uncached deep chain follows the new link", deep_built && selftest_ok(VM_OBJ_SCALAR_GET(after, deep)) && after == 901);
  ck("cached accessor still names its own object", selftest_ok(VM_OBJ_SCALAR_GET(v, elem)) && v == 777);

  /* A cached accessor and a fresh uncached one must never disagree. */
  vm_accessor_t* twin       = NULL;
  bool           twin_built = selftest_ok(vm_accessor_create(&twin, s_acc_id++, 1, 1)) && selftest_ok(vm_accessor_set_literal(twin, 0, 2));
  uint32_t       a = 0, b = 0;
  ck("cached and uncached agree", twin_built && selftest_ok(VM_OBJ_SCALAR_GET(a, elem)) && selftest_ok(VM_OBJ_SCALAR_GET(b, twin)) && a == b);
  ...
  ck("a dropped cache falls back to failing closed", !selftest_ok(VM_OBJ_SCALAR_GET(v, &detached)));
