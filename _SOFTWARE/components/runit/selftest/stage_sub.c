...
void test_subscription(void) {
  ...
  ck("sub: open program", selftest_ok(upload_open(4, 4, 1, 1024)));
  ...
  ck("sub: create objects", selftest_ok(f_send()));
  ...
  ck("sub: set initial data", selftest_ok(f_send()));
  ...
  f_u8(1);
  f_u16(1);
  ck("sub: 0x47 subscribe to obj 1", selftest_ok(f_send()) && vm_sub_count() == 1);
  ...
  f_u8(1);
  f_u16(3);
  ck("sub: subscribe to nested obj 3", selftest_ok(f_send()) && vm_sub_count() == 1);
  ...
  f_begin(VM_LOADER_CLASS_HEADER, 0x47);
  f_u8(0);
  ck("sub: unsubscribe all via count 0", selftest_ok(f_send()) && vm_sub_count() == 0);
