...
void test_block_support(void) {
  ...
  *(uint8_t*)gate->payload  = 0;
  ...
  ck("disabled timer resets without resolving signal", !g_vm_block_fault && !timer.elapsed_ms && !(timer.flags & VM_TIMER_F_INITIALIZED) && *(uint8_t*)output->payload == 0);
