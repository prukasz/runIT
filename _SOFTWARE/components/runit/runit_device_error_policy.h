...
// Internal action reference: bit 8 selects dynamic scope; low byte is the ID.
// Zero disables the action. No fallback between namespaces.
static err_h runit_invoke_action(uint16_t action) {
  if (!(action & SYS_DEV_ACTION_ID_MASK)) return NULL;
  return sys_actions_invoke((action & SYS_DEV_ACTION_DYNAMIC) ? SYS_ACTION_SCOPE_DYNAMIC : SYS_ACTION_SCOPE_STATIC, action & SYS_DEV_ACTION_ID_MASK);
}
...
err_h sys_device_app_error_policy(uint8_t device_id, sys_device_err_level_e level, uint16_t action_id, err_h error) {
  err_h response = level == SYS_DEV_ERR_CRITICAL ? runit_system_fault(device_id, error) : NULL;
  err_h action   = runit_response_error(device_id, level, SYS_DEV_FAULT_STAGE_ACTION, action_id, runit_invoke_action(action_id));
