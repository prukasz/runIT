...
class SubscriptionBuilder:
    ...
    ----------
    code : Code
        Code object that owns ``mem`` for alias resolution.
    pkt_size : int
    ...
    def add(self, target: Union[str, Ref]) -> 'SubscriptionBuilder':
        ...
        Resolves through Mem to find context, type & index.
        """
        alias = target.alias if isinstance(target, Ref) else target
        inst = self._mem.get_instance(alias)
        ctx_id = inst.head.context
        m_type = mem_types_t(inst.head.mem_type)
        idx = inst.my_index
